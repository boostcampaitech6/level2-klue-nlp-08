...
chmod +x upload_data_to_GC.sh 혹은 chmod +x upload_model_to_GC.sh
이후 아래 명령어를 치면 실행된다.
./upload_data_to_GC.sh 혹은 ./upload_model_to_GC.sh
...
아래 명령어를 순차적으로 실행한다.
pip install gdown
chmod +x download_data_from_GC.sh 혹은 chmod +x download_model_from_GC.sh
이후 아래 명령어를 치면 실행된다.
